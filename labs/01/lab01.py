...
def median_vs_average(nums):
    ...
    True
    >>> median_vs_average([0, -1, 1, 100])
    False
    >>> median_vs_average([-50,-20,0,20,50])
    True
    """
    ...
    if len(nums) % 2 == 0:
        ...
        mid2 = int(len(nums) / 2)
        median = (sort_nums[mid1] + sort_nums[mid2]) / 2
    else:
        median = sort_nums[int(len(nums) / 2)]
...
def exploded_numbers(ints, n):
    ...
    explosion = []
    zero_pad = len(str(max(ints)))
    for num in ints:
        string = ""
        left = n
        while left >= 0:
            string += str(num - left).zfill(zero_pad) + " "
            left -= 1

        right = 1
        while right <= n:
            string += str(num + right).zfill(zero_pad) + " "
            right += 1
        explosion.append(string[:-1])
...
def arr_1(A):
    ...
    return A + (np.argwhere(A) ** 0.5).reshape(len(A))
...
def arr_4(A):
    ...
    :returns: the day on which you can buy at least one share from 'left-over'
    money
    :Example:
    >>> import numbers
    ...
    True
    """
    twenty = np.ones(len(A)) * 20
    left_over = np.mod(twenty, A)
    days_to_spend = np.cumsum(left_over) >= A
    if np.all(days_to_spend == False):
        return -1
    day = np.where(days_to_spend == True)[0][0]
    return day
...
def salary_stats(salary):
    ...
    vals = []
    i = ['num_players', 'num_teams', 'total_salary', 'highest_salary',
         'avg_bos', 'third_lowest', 'duplicates', 'total_highest']

    vals.append(salary['Player'].count())
    vals.append(len(salary['Team'].unique()))
    vals.append(salary['Salary'].sum())
    vals.append(salary.sort_values('Salary').iloc[-1].get('Player'))
    vals.append(np.round(salary[salary['Team'] == 'BOS'].get('Salary').mean(),
                         2))
    vals.append(salary.sort_values('Salary').iloc[2][0] + ", " +
                salary.sort_values('Salary').iloc[2][1])
    vals.append()
    return pd.Series(vals, index=i)
