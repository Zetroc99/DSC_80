...
def match_9(string):
    ...
    pattern = '(CA-\d{2}-(SAN|LAX)-\d{4})|(NY-\d{2}-[A-Z]{3}-\d{4})'
...
def match_10(string):
    ...
    s = re.findall('[^a]..|[\w\s]{3}', string)
    s = list(filter(lambda string: '' if string[0] == 'a' else string, s))

    return list(map(lambda string: re.sub('[^\w\s]', '', string), s))
...
def extract_personal(s):
    ...
    b = re.findall('[\w]*', s)
    bits = list(filter(lambda x: len(x) > 20 and len(x) < 50, b))
